...
static int newDbSaveTable( char * tablename, char * data, int len ) {//-把内容保存到数据库指定的文件中
    ...
    if ( tablename && data && len > 0 ) {
        ...
        if ( stat(filename, &sb) == -1) {//-通过文件名filename获取文件信息，并保存在buf所指的结构体stat中,执行成功则返回0，失败返回-1
            DEBUG_PRINTF( "No dB file found. Stat %s, errno = %d\n", filename, errno );
        } else {//-成功了,说明有,所以可以删除备份了,正好更新后,原来的就是备份了
            ...
            if ( rename( filename, backup ) != 0 ) {//-给一个文件重命名，用该函数可以实现文件移动功能，把一个文件的完整路径的盘符改一下就实现了这个文件的移动。
        ...
        if ( fileCreateRW( filename ) ) {//-再次创建了数据库文件
 ...
 * \returns 1 on success, 0 on error
 */
static int newDbRestoreTable( char * tablename, char * data, int len ) {//-恢复某个表格
