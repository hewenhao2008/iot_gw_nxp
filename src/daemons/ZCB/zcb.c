...
int zcbAddNode( uint16_t  shortAddress, uint64_t extendedAddress ) {//-当下面有会说明端点消息上来,然后触发更新数据库
    ...
    newdb_zcb_t zcb;
    if ( newDbGetZcbSaddr( shortAddress, &zcb ) ) {//-如果存在这样的短地址就更新一次mac地址
        zcb.status = ZCB_STATUS_JOINED;
        strcpy( zcb.mac, mac );
        newDbSetZcb( &zcb );
        
    } else if ( newDbGetZcb( mac, &zcb ) ) {//-如果存在这样的mac地址就更新一次短地址
        zcb.status = ZCB_STATUS_JOINED;
...
static void ZCB_HandleDeviceAnnounce(void *pvUser, uint16_t u16Length, void *pvMessage) {//-终端设备发送的通告
