 ...
 * Creates the node is the database if required.
 * @param shortAddress
 * @param extendedAddress
 * @return 1 if a new node is created, 0 else
 */
int zcbAddNode( uint16_t  shortAddress, uint64_t extendedAddress ) {//-当一个新的端点加入网络的时候更新数据库
    ...
    newdb_zcb_t zcb;
    if ( newDbGetZcbSaddr( shortAddress, &zcb ) ) {//-如果存在这样的短地址就更新一次mac地址,在一个局域网络中短地址也是唯一的
        zcb.status = ZCB_STATUS_JOINED;
        strcpy( zcb.mac, mac );
        newDbSetZcb( &zcb );
        
    } else if ( newDbGetZcb( mac, &zcb ) ) {//-如果存在这样的mac地址就更新一次短地址,在整个网络中mac地址是唯一的
        zcb.status = ZCB_STATUS_JOINED;
        ...
        zcb.status = ZCB_STATUS_JOINED;
        zcb.saddr  = shortAddress;	//-这里记录的不是zcb地址,而是终端设备地址
...
int zcbNodeSetDeviceID( uint16_t shortAddress, uint16_t deviceID ) {//-其实就是向数据库中继续填写需要的数据
...
teZcbStatus eZCB_EstablishComms(void) {//-最初开始的地方,程序一旦运行起来就会这么干
    if (eSL_SendMessage(E_SL_MSG_GET_VERSION, 0, NULL, NULL) == E_SL_OK) {
        ...
        if (eSL_MessageWait(E_SL_MSG_VERSION_LIST, 300, &u16Length, (void**)&u32Version) == E_SL_OK) {
            ...
            newDbSystemSaveIntval( "zcb_version", version );	//-这个数值是保存在数据库中的,应该有一个识别作用
    ...
    if (psManagementLQIResponse->u8Status == CZD_NW_STATUS_SUCCESS)
    {
      ...
      for (i = 0; i < psManagementLQIResponse->u8TableEntries; i++)
      {
        ...
        if ( newDbGetZcbSaddr( psManagementLQIResponse->asNeighbours[i].u16ShortAddress, &zcb ) ) {
          ...
          // Handle as teh device announced itself
...
//-Node->Host,当dongle接收到复位命令之后就会主动发送这个消息,那么数据库中就有了dongle的信息,而每次连接dongle都会首先复位下
static void ZCB_HandleNetworkJoined(void *pvUser, uint16_t u16Length, void *pvMessage) {//-加入网络或建立网络的处理
    ...
    zcbAddNode( psMessageShort->u16ShortAddress, psMessageShort->u64IEEEAddress );	//-每个设备用一个zcb记录
    
    char  ePanId[16+2];
    if ( psMessageShort->u16ShortAddress == 0x0000 ) {
        // Coordinator
        u642nibblestr( psMessageShort->u64IEEEAddress, ePanId );
        newDbSystemSaveStringval( "zcb_extpanid", ePanId );	//-在数据库中特别记录下协调器的信息,减少了查库
        zcbNodeSetDeviceID( psMessageShort->u16ShortAddress, SIMPLE_DESCR_GATEWAY );	//-由于确认是协调器,所以需要填写这样的设备类型,在数据库中
        eZCB_AddGroupMembership( 0x0000, 0x0AA1 );	//-这个是放到什么组里面了,在没有用到之前是没有影响的
    }
...
static void ZCB_HandleDeviceAnnounce(void *pvUser, uint16_t u16Length, void *pvMessage) {//-终端设备发送的通告,一个终端设备加网后就会主动申明不管是第一还是重新上电
    ...
    zcbAddNode(psMessage->u16ShortAddress, psMessage->u64IEEEAddress);	//-在通告报文内包含终端设备的短地址,和mac地址
    ...
    void *pvMessage)
{
  ...
  if (!newDbGetZcbSaddr(u16ShortAddr, &sZcbDev))	//-判断应答的报文的短地址是否在zcb中
  {
