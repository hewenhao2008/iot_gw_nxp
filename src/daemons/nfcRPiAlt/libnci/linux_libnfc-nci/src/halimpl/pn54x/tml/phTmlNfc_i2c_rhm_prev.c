...
#define I2C_BUS            ALT_NFC_I2C_BUS       // "/dev/i2c-1"
#define I2C_ADDRESS        ALT_NFC_I2C_ADDR      // 0x28
#define I2C_SPEED          100000

#define PIN_INT            ALT_NFC_GPIO_INT      // 23
#define PIN_ENABLE         ALT_NFC_GPIO_ENABLE   // 24
...
static int verifyPin( int pin, int isoutput, int edge ) {
	...
	if ( fd <= 0 ) {
		...
		if ( write(fd,"in",2) == 2 ) {
		    DEBUG_PRINTF( "Pin %d now an input\n", pin );
		}
		close(fd);
		...
		if ( edge != EDGE_NONE ) {
		    ...
		    fd = open( buf, O_RDWR );
		    if ( fd <= 0 ) {
			printf( "Could not open edge port '%s' (%s)",
				buf, strerror(errno) );
		    } else {
			char * edge_str = "none";
			switch ( edge ) {
			...
			if ( write( fd, edge_str, strlen(edge_str) ) == strlen(edge_str) ) {
			    DEBUG_PRINTF( "Pin %d trigger on %s\n", edge_str );
			}
			close(fd);
		    }	
		}
...
static int pnGetint( void ) {
    ...
    if (iInterruptFd <= 0) {
	printf( "Error with interrupt-detect pin (%d)", iInterruptFd );
	return( -1 );
    }
    ...
    if (len != 2) {
	printf( "Error with interrupt-detect pin (%s)", strerror(errno));
	return( 0 );
    }
...
NFCSTATUS phTmlNfc_i2c_open_and_configure(pphTmlNfc_Config_t pConfig, void ** pLinkHandle)
{
  ...
  DEBUG_PRINTF( "RHM: NFCHW - open and config - %s\n", pConfig->pDevName);
    ...
    if (nHandle < 0)
    {
  DEBUG_PRINTF( "RHM: NFCHW - open and config - 1\n" );
  ...
  DEBUG_PRINTF( "RHM: NFCHW - open and config - 2\n" );
...
// sudo reboot
  ...
  DEBUG_PRINTF( "RHM: NFCHW - open I2C device - 0x%02x\n", I2C_ADDRESS);

    // I2C slave address
    if (ioctl(iI2CFd, I2C_SLAVE, I2C_ADDRESS) < 0) {
	DEBUG_PRINTF( "Cannot select I2C address (%s)", strerror(errno));
	if ( iEnableFd    ) close(iEnableFd);
	if ( iInterruptFd ) close(iInterruptFd);
	close(iI2CFd);
	return( NFCSTATUS_INVALID_DEVICE );
...
int phTmlNfc_i2c_read(void *pDevHandle, uint8_t * pBuffer, int nNbBytesToRead)
{
  ...
  pDevHandle = iI2CFd;
    ...
    // TODO RHM : Probably use original code above
	...
	// Read the header
	wait4interrupt();
	ret_Read = read(iI2CFd, pBuffer, totalBtyesToRead );
	if (ret_Read == totalBtyesToRead) {
	    numRead += ret_Read;
	} else if (ret_Read == 0) {
	    printf( "I2c-Read: Error reading header (EOF)\n" );
	    return -1;
	} else {
	    // printf( "I2c-Read: Error reading header (%d)\n", ret_Read );
	    return -1;
	}

	// Look in header to see what follows
	if(TRUE == bFwDnldFlag) {
	    totalBtyesToRead = pBuffer[FW_DNLD_LEN_OFFSET] + CRC_LEN;
	} else {
	    totalBtyesToRead = pBuffer[NORMAL_MODE_LEN_OFFSET];
	}
	wait4interrupt();
	ret_Read = read(iI2CFd, (pBuffer + numRead), totalBtyesToRead );
	if (ret_Read == totalBtyesToRead) {
	    numRead += ret_Read;
	} else if (ret_Read == 0) {
	    printf( "I2c-Read: Error reading rest (EOF)\n" );
	    return -1;
	} else {
	    printf( "I2c-Read: Error reading rest (%d)\n", ret_Read );
	    return -1;
	}
...
int phTmlNfc_i2c_write(void *pDevHandle, uint8_t * pBuffer, int nNbBytesToWrite)
{
  ...
  pDevHandle = iI2CFd;
    ...
    // TODO RHM : Support fragmentation
    // Probably use original code above
    
    int len = write(iI2CFd, pBuffer, nNbBytesToWrite);
    if ( len > 0 ) {
      DEBUG_PRINTF("RHM: Wrote %d bytes\n", len );
    } else {
      printf("RHM: Write error %d, %d, %s\n", iI2CFd, nNbBytesToWrite, strerror(errno) );
    }
    return( len );
...
{
  DEBUG_PRINTF( "RHM: NFCHW - reset level %d\n", level );
